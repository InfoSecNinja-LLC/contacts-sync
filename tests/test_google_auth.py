...
def test_run_installed_app_auth_saves_credentials(mocker):
    ...
    save_mock = mocker.patch("contacts_sync.auth.google_auth.env_set")

    google_auth.run_installed_app_auth("client_secrets.json")

    save_mock.assert_any_call("GOOGLE_REFRESH_TOKEN", "rt-1")
    save_mock.assert_any_call("GOOGLE_CLIENT_ID", "cid-1")
    save_mock.assert_any_call("GOOGLE_CLIENT_SECRET", "secret-1")


def test_get_credentials_raises_when_not_authed(mocker):
    mocker.patch("contacts_sync.auth.google_auth.env_read", side_effect=EnvStoreError("nope"))
...
def test_get_credentials_builds_and_refreshes(mocker):
    mocker.patch(
        "contacts_sync.auth.google_auth.env_read",
